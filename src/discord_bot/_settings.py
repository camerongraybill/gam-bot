...
class _Settings:
    ...
    def COGS(self) -> Collection[Type[BaseCog]]:
        ...
        for cog_path in getattr(django_settings, 'DISCORD_COGS', []):
            ...
            try:
                ...
                raise Exception(f"Attempted to import module {module_path} from DISCORD_MODULES, but it does not exist.")
            if not hasattr(module, cog_name):
                raise Exception(f"Attempted to import cog named {cog_name} from module {module_path} but {module_path} has no attribute {cog_name}")

            cls = getattr(module, cog_name)
            if not issubclass(cls, BaseCog):
                raise Exception(f"Attempted to set up cog {cog_path} but it is not a subclass of discord_bot.BaseCog")
    ...
    def KEY(self) -> Optional[str]:
        return getattr(django_settings, 'DISCORD_KEY', None)

    @cached_property
    def COMMAND_PREFIX(self) -> str:
        return getattr(django_settings, 'DISCORD_COMMAND_PREFIX', '!')
